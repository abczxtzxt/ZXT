...
public class ICReaderApi {
    ...
    public boolean isOk() {
        return device != null && manager != null && transfer != null;
    }

    private UsbDevice device;
    private UsbManager manager;

    private static class ICReaderApiHolder {
        private static ICReaderApi instance = new ICReaderApi();
    }

    public static ICReaderApi getInstance(UsbDevice device, UsbManager manager) {
        if (ICReaderApiHolder.instance.device == null) {
            ICReaderApiHolder.instance.device = device;
        }
        if (ICReaderApiHolder.instance.manager == null) {
            ICReaderApiHolder.instance.manager = manager;
        }

        if (ICReaderApiHolder.instance.transfer == null) {
            ICReaderApiHolder.instance.transfer = new Transfer(device, manager);
        }

        return ICReaderApiHolder.instance;
    }
